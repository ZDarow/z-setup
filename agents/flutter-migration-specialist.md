...
### Required Extensions for Migration Work
```bash
# Установка критических расширений для Flutter-миграции
code --install-extension Dart-Code.dart-code
code --install-extension Dart-Code.flutter
code --install-extension bradlc.vscode-tailwindcss
code --install-extension ms-vscode.android-sdk
code --install-extension ms-vscode.java-dependency
code --install-extension formulahendry.dotenv
code --install-extension 1yi1code.vscode-jadx

# Проверка установленных
code --list-extensions | grep -E "(Dart|Flutter|Android)"
```

### Automated VS Code Setup via vscode-setup Skill
```bash
# Автоматическая настройка через навык
# Агент вызовет vscode-setup для Flutter-проекта

1. Анализ проекта: code --list-extensions
2. Создание .vscode/extensions.json:
{
    "recommendations": [
        "dart-code.dart-code",
        "dart-code.flutter",
        "ms-vscode.android-sdk",
        "bradlc.vscode-tailwindcss",
        "1yi1code.vscode-jadx"
    ]
}
3. Создание .vscode/settings.json:
{
    "dart.flutterSdkPath": "/home/mi/flutter",
    "dart.sdkPath": "/home/mi/flutter/bin/cache/dart-sdk",
    "flutter.selectDeviceWhenConnected": true,
    "dart.closingLabels": true
}
```

### Extension-Specific Migration Tools

#### Jadx (Android Reverse Engineering)
```bash
# Использование Jadx через VS Code для анализа Android-приложения
# 1. Открыть APK в Jadx
code --add /path/to/decompiled/android_analysis/

# 2. Поиск Flutter-специфичных классов
# Искать: FlutterActivity, FlutterApplication, MethodChannel

# 3. Извлечение логики для миграции
# Через VS Code: references/, search, outline
```

#### Android SDK Extension
```bash
# Настройка Android SDK для Flutter-миграции
# .vscode/settings.json:
{
    "android.sdkPath": "/home/mi/Android/Sdk",
    "android.buildToolsVersion": "34.0.0",
    "android.compileSdkVersion": 34
}

# Проверка через VS Code Terminal:
adb devices  # Проверка подключенных устройств
flutter doctor  # Диагностика Flutter-окружения
```
...
- Настройка VS Code для Flutter-разработки (расширения, конфиги)
- Автоматическая установка необходимых плагинов через VS Code CLI
